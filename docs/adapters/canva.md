...
2. Get a design to fill. Ask the person which they want; do not decide for them:
   - **Their own pick.** A Canva resume template they chose in the Canva UI (Create a design,
     Resumes), or a design they already own. They paste the design id from the URL into
     `career.json` `cv.canva_design_id`. The usual answer.
   - **Generated by AI.** `generate-design-structured` with a brief built from cv-canonical
     (role count, sections, one page or two, colour and tone from their words), then
     `create-design-from-candidate` on the one they like. Ask for the brief in their words
     first. Check the result before filling: text in text boxes, not images; one or two
     columns; no icon-only sections. Two-column and heavily decorated layouts parse badly in
     ATS, so say so once and let them choose.
   - **A copy of a working design.** `copy-design` of a CV that already works with this
     adapter (a coach's own, a partner's, an earlier client's; the text is replaced in step
     3). The shortcut when they want the result today and do not care about the look yet.
   Whatever they pick, run the export through `pdftotext` once before filling: if the text
   comes out empty or scrambled, the template is not ATS-readable, pick another.
