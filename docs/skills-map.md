...
|-------|-------------|-------|---------------------------------|
| Start | `intake` (new person) or `career-setup` (tools only) | `profile-fetch`, `profile-review` | you only need tools installed |
...
| Visibility | `linkedin-growth` (the plan and routine) | `content-engine` (production), `linkedin-post` (one post), `post-visuals`, `case-study`, `talks-and-writing`, `video-intro`, `recommendations`, `headline-test` | you want one artifact, not the plan |
...
| Unattended | `career-cron` | `inbox-sync`, `notify`, `operator-selftest`, `voice-notes`, `notes-sync` | a single run |
